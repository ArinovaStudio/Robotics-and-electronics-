...
const PriceRangeSlider: React.FC<PriceRangeSliderProps> = ({ min, max, value, onChange }) => {
  ...
  return (
    <div className="px-1 py-2">
      <div ref={trackRef} className="relative mx-2.5 my-2.5" style={{ height: 3, background: "#e0e0e0", borderRadius: 2 }}>
        <div className="absolute h-full" style={{ left: `${pct(value[0])}%`, right: `${100 - pct(value[1])}%`, background: "#f0b31e", borderRadius: 2 }} />
        <div onMouseDown={startDrag("low")} className="absolute cursor-grab active:cursor-grabbing select-none" style={{ top: "50%", left: `${pct(value[0])}%`, transform: "translate(-50%, -50%)", width: 18, height: 18, borderRadius: "50%", background: "#f0b31e", border: "2.5px solid #f0b31e", boxShadow: "0 0 0 2px #fff, 0 2px 6px rgba(0,0,0,0.18)", zIndex: 3 }} />
        <div onMouseDown={startDrag("high")} className="absolute cursor-grab active:cursor-grabbing select-none" style={{ top: "50%", left: `${pct(value[1])}%`, transform: "translate(-50%, -50%)", width: 18, height: 18, borderRadius: "50%", background: "#f0b31e", border: "2.5px solid #f0b31e", boxShadow: "0 0 0 2px #fff, 0 2px 6px rgba(0,0,0,0.18)", zIndex: 3 }} />
      </div>
...
const CustomCheckbox: React.FC<{ checked: boolean; onChange: () => void }> = ({ checked, onChange }) => (
  <span onClick={onChange} className="inline-flex items-center justify-center flex-shrink-0 cursor-pointer transition-all duration-100" style={{ width: 17, height: 17, minWidth: 17, border: `1px solid ${checked ? "#f0b31e" : "#B1B1B1"}`, borderRadius: 2, background: checked ? "#f0b31e" : "#fff" }}>
    {checked && (
...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ onFiltersChange, availableBrands = [], resetKey = 0 }) => {
  ...
  const [selectedPriceRange, setSelectedPriceRange] = useState<[number, number]>([0, 50000]); // Max 50,000 INR
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      onFiltersChange({
        ...
        maxPrice: selectedPriceRange[1] >= 50000 ? 999999 : selectedPriceRange[1], // If maxed, remove upper limit
      });
    }, 400);
      ...
      <div className="mb-2">
        ...
        <div className="flex flex-col gap-3">
          {visibleCategories.map((c) => (
            <label key={c.id} className="flex items-center gap-3 text-sm text-gray-800 font-medium cursor-pointer select-none hover:text-[#f0b31e] transition-colors">
              <CustomCheckbox
        ...
        {categories.length > 5 && (
          <button onClick={() => setShowAllCategories(!showAllCategories)} className="mt-4 block text-xs font-bold text-gray-400 hover:text-[#f0b31e] transition-colors">
