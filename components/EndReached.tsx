...
const EndReached: React.FC<{ onScrollTop?: () => void }> = ({
  ...
  return (
    <div className="w-full flex items-center justify-between bg-[#f0b31e]  p-12 relative overflow-hidden min-h-[340px]">
      ...
      <div className="z-10">
        <h2
          className={`text-white text-5xl font-bold mb-6 tracking-tight ${spaceGrotesk.className}`}
        >
          END REACHED
        </h2>
        <p className="text-white text-2xl  mb-8 max-w-3xl">
          You have browsed all the product in this category, you may try to
          change the filters and catrgory to browse more!
        </p>
        <button
          className={`bg-white text-[#f0b31e] cursor-pointer font-bold text-xl px-8 py-3 shadow-lg hover:shadow-xl transition-all duration-150 border-none outline-none ${spaceGrotesk.className}`}
      ...
      <div className="absolute right-0 top-1/2 -translate-y-1/2 h-full flex items-center justify-end z-0">
        ...
        <div className="w-[350px] h-[350px] flex items-center justify-end">
          <img
            src="/circleSVG.svg"
            alt="Circles SVG"
