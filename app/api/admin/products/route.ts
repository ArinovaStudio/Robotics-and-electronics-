...
export async function GET(request: NextRequest) {
  try {
    ...
    const [products, totalFiltered, allProductsForMetrics] = await Promise.all([
      ...
      prisma.product.findMany({
        select: { stockQuantity: true, price: true, isActive: true },
      }),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!imageFile || imageFile.size === 0)
      return errorResponse("Primary image is required", 400);
    ...
    const additionalImageLinks =
      validAdditionalFiles.length > 0
        ? (await uploadMultipleFiles(validAdditionalFiles, "products")).map(
            (f) => f.path,
          )
        : [];
