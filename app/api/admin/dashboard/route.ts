...
export async function GET() {
  try {
    ...
    // Recent stats
    const [
      ordersTodayCount, revenueTodayData,
      ordersWeekCount, revenueWeekData,
      ordersMonthCount, revenueMonthData,
    ] = await Promise.all([
      prisma.order.count({ where: { orderedAt: { gte: startOfToday } } }),
      prisma.order.aggregate({ _sum: { totalAmount: true }, where: { orderedAt: { gte: startOfToday }, status: { in: ["CONFIRMED", "PROCESSING", "SHIPPED", "DELIVERED"] } } }),
      prisma.order.count({ where: { orderedAt: { gte: startOfWeek } } }),
      prisma.order.aggregate({ _sum: { totalAmount: true }, where: { orderedAt: { gte: startOfWeek }, status: { in: ["CONFIRMED", "PROCESSING", "SHIPPED", "DELIVERED"] } } }),
      prisma.order.count({ where: { orderedAt: { gte: startOfMonth } } }),
      prisma.order.aggregate({ _sum: { totalAmount: true }, where: { orderedAt: { gte: startOfMonth }, status: { in: ["CONFIRMED", "PROCESSING", "SHIPPED", "DELIVERED"] } } }),
    ]);
    ...
    // Orders by status
    const orderStatusCounts = await prisma.order.groupBy({ by: ["status"], _count: { id: true } });
    const ordersByStatus = { PENDING: 0, CONFIRMED: 0, PROCESSING: 0, SHIPPED: 0, DELIVERED: 0, CANCELLED: 0 };
    orderStatusCounts.forEach((item) => {
      ordersByStatus[item.status as keyof typeof ordersByStatus] = item._count.id;
    });
    ...
    const products = await prisma.product.findMany({
      where: { id: { in: topSellingProductIds } }, select: { id: true, title: true },
    });
    ...
    const chartOrders = await prisma.order.findMany({
      where: { orderedAt: { gte: oneYearAgo }, status: { in: ["CONFIRMED", "PROCESSING", "SHIPPED", "DELIVERED"] } },
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      weeklyMap.set(d.toISOString().split('T')[0], { date: d.toLocaleDateString('en-US', {weekday: 'short'}), sale: 0, purchase: 0 });
    }
    // Init Month (Last 4 weeks)
    for (let i = 3; i >= 0; i--) {
      monthlyMap.set(`Week ${4-i}`, { date: `Week ${4-i}`, sale: 0, purchase: 0 });
    }
    // Init Year (Last 12 months)
    for (let i = 11; i >= 0; i--) {
      const d = new Date(); d.setMonth(d.getMonth() - i);
      yearlyMap.set(`${d.getFullYear()}-${d.getMonth()}`, { date: d.toLocaleDateString('en-US', {month: 'short'}), sale: 0, purchase: 0 });
    }
    ...
    chartOrders.forEach(order => {
       ...
       // Monthly
       const daysAgo = Math.floor((now.getTime() - order.orderedAt.getTime()) / (1000 * 3600 * 24));
       if (daysAgo <= 28) {
